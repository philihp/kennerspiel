...
> **Engine roster note.** In *this* engine, the **2-player game (short or long) uses 24 France buildings** — only cards whose **P** column contains a `2`. Seventeen France cards are therefore absent from 2-player play: **G01, F03, G06, F09, F10, G13** (Start), **F15, G16** (A), **F20, F23, F25** (B), **F29, F31, F32** (C), **F36, G39, F40** (D). (The printed rulebook says the *long* 2-player game should use nearly all buildings — only F10 Carpentry, F31 C-Grapevine, F29 C-Quarry removed — so this engine diverges from the printed long-2p rule; treat the **P** column as authoritative for what actually appears here.)
...
11. **Engine ≠ printed long-2p roster.** This engine's 2-player game uses the 24-building set (P-column contains `2`); the printed long-2p "nearly all buildings" rule is not implemented here.
