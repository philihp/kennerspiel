...
**USE and BUILD both require a free clergyman — running out of clergy locks you out of building.** With all your clergy placed, the only legal verbs are WORK_CONTRACT / BUY_PLOT / BUY_DISTRICT / CONVERT, and you cannot even COMMIT to pass — you must spend a no-clergy action (a contract, a purchase) just to cycle your clergy home next turn. Consequence: if your plan is to BUILD the display down to force termination, you must *reserve a free clergyman* for the build. Track clergy as a build resource, not only a use resource, and never strand yourself clergy-locked on the turn you need to build.

**Work-contract is a denial weapon, not just a rental.** Contracting an opponent's building forces THEM to seat one of THEIR clergymen on it. So contracting the engine building they lean on (their Workshop) does three things at once: you take the output, you deny them the building's use, and you push them toward the clergy lockout above. Red shut my freshly re-lit +9 Workshop off with a single `WORK_CONTRACT G18` at the exact moment I needed it. Aim contracts at whatever the opponent is currently leaning on, and time them for the turn they most need it free.
