...
describe('game-solo-settle', () => {
  it('settling should', () => {
    ...
    const s02 = reducer(s01, ['CONFIG', '1', 'france', 'long'])! as GameState
    const s03 = reducer(s02, ['START', 'G', 'W'])! as GameState
    expect(s03.players!.map((p) => p.color)).toStrictEqual(['G', 'W'])
    expect(s03).toBeDefined()
    const s06 = reducer(s03, ['USE', 'LG1'])! as GameState
    expect(s06).toBeDefined()
    const s07 = reducer(s06, ['COMMIT'])! as GameState
    const s08 = reducer(s07, ['FELL_TREES', '2', '0'])! as GameState
    expect(s08).toBeDefined()
    const s09 = reducer(s08, ['COMMIT'])! as GameState
    expect(s09).toBeDefined()
    const s10 = reducer(s09, ['USE', 'LG2', 'Gn'])! as GameState
    expect(s10).toBeDefined()
    const s11 = reducer(s10, ['CONVERT', 'Gn'])! as GameState
    expect(s11).toBeDefined()
    const s12 = reducer(s11, ['COMMIT'])! as GameState
    const s13 = reducer(s12, ['CUT_PEAT', '0', '0'])! as GameState
    const s14 = reducer(s13, ['COMMIT'])! as GameState
    const s15 = reducer(s14, ['BUILD', 'G07', '0', '0'])! as GameState
    const s16 = reducer(s15, ['USE', 'G07', 'PtPtPt'])! as GameState
    const s17 = reducer(s16, ['COMMIT'])! as GameState
    const s18 = reducer(s17, ['FELL_TREES', '1', '1'])! as GameState
    const s19 = reducer(s18, ['COMMIT'])! as GameState
    const s20 = reducer(s19, ['BUILD', 'F03', '1', '1'])! as GameState
    const s21 = reducer(s20, ['USE', 'F03', 'Pn'])! as GameState
    const s22 = reducer(s21, ['COMMIT'])! as GameState
    const s23 = reducer(s22, ['USE', 'LG1'])! as GameState
    const s24 = reducer(s23, ['COMMIT'])! as GameState
    const s25 = reducer(s24, ['CONVERT', 'Gn'])! as GameState
    expect(s25).toBeDefined()
    const s26 = reducer(s25, ['BUILD', 'G06', '2', '0'])! as GameState
    const s27 = reducer(s26, ['COMMIT'])! as GameState
    const s28 = reducer(s27, ['USE', 'G06', 'CoCoCo'])! as GameState
    const s29 = reducer(s28, ['COMMIT'])! as GameState
    const s30 = reducer(s29, ['BUY_DISTRICT', '2', 'HILLS'])! as GameState
    const s31 = reducer(s30, ['BUILD', 'F04', '3', '2'])! as GameState
    const s32 = reducer(s31, ['USE', 'F04', 'GnGnGnGnGnGnGn'])! as GameState
    const s33 = reducer(s32, ['COMMIT'])! as GameState
    const s34 = reducer(s33, ['USE', 'F03', 'Pn'])! as GameState
    const s35 = reducer(s34, ['COMMIT'])! as GameState
    const s36 = reducer(s35, ['BUILD', 'G01', '3', '1'])! as GameState
    const s37 = reducer(s36, ['COMMIT'])! as GameState
    const s40 = reducer(s37, ['CUT_PEAT', '0', '2'])! as GameState
    const s41 = reducer(s40, ['COMMIT'])! as GameState
    const s42 = reducer(s41, ['FELL_TREES', '1', '0'])! as GameState
    expect(s42.players![0].landscape[0][3]).toStrictEqual(['P'])
    const s43 = reducer(s42, ['COMMIT'])! as GameState
    const s44 = reducer(s43, ['USE', 'G07', 'PtPtPtPtPtPt'])! as GameState
    const s45 = reducer(s44, ['COMMIT'])! as GameState
    const s46 = reducer(s45, ['USE', 'G06', 'CoCoCo'])! as GameState
    const s47 = reducer(s46, ['BUY_DISTRICT', '3', 'PLAINS'])! as GameState
    ...
    const s49 = reducer(s48, ['BUILD', 'F09', '4', '2'])! as GameState
    ...
    const s50 = reducer(s49, ['USE', 'F09'])! as GameState
    ...
    const s51 = reducer(s50, ['USE', 'LG3'])! as GameState
    ...
    const s53 = reducer(s52, ['USE', 'LG1'])! as GameState
    expect(s53).toBeDefined()
    const s54 = reducer(s53, ['COMMIT'])! as GameState
    expect(s54).toBeDefined()
    expect(s54.players![0]).toMatchObject({
      penny: 10,
    })
    expect(s54.players![1].landscape[0][2]).toStrictEqual(['P', 'G13'])
    const s55 = reducer(s54, ['WORK_CONTRACT', 'G13', 'Pn'])! as GameState
    expect(s55).toBeDefined()
    const s56 = reducer(s55, ['USE', 'G13', 'PnPn'])! as GameState
    const s57 = reducer(s56, ['COMMIT'])! as GameState
    expect(s57).toBeDefined()
    const s58 = reducer(s57, ['BUILD', 'F05', '2', '3'])! as GameState
    const s59 = reducer(s58, ['USE', 'F05', 'FlFlFlFlFlFlFlCoCoBrBr'])! as GameState
    const s59a = reducer(s59, ['COMMIT'])! as GameState
    const s60 = reducer(s59a, ['BUY_PLOT', '2', 'COAST'])! as GameState
    const s61 = reducer(s60, ['BUILD', 'F11', '-1', '2'])! as GameState
    ...
    const s62 = reducer(s61, ['COMMIT'])! as GameState
    ...
    const s63 = reducer(s62, ['BUILD', 'G02', '3', '1'])! as GameState
    ...
    const s64 = reducer(s63, ['BUILD', 'F08', '1', '0'])! as GameState
    ...
    const s65 = reducer(s64, ['BUILD', 'G12', '2', '0'])! as GameState
    expect(s63.frame).toMatchObject({
      nextUse: NextUseClergy.OnlyPrior,
    })
    ...
    const s65b = reducer(s65, ['WORK_CONTRACT', 'G12', 'Pn'])! as GameState
    ...
    const s66 = reducer(s65b, ['USE', 'G12', 'BrBrBrPnCoCo'])! as GameState
    ...
    const s67 = reducer(s66, ['SETTLE', 'SG2', '3', '3', 'BrCo'])! as GameState
    const c67a = control(s67, [])
    expect(c67a.completion).toStrictEqual(['BUY_PLOT', 'BUY_DISTRICT', 'CONVERT', 'COMMIT'])

    const s68 = reducer(s67, ['COMMIT'])! as GameState
